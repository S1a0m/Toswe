export function goToSearch()  {return navigateTo({ path: "/search" })}
export function goToAuth()  {return navigateTo({ path: "/auth" })}
export function goToMarket()  {
    return navigateTo({ path: "/market" })
}
export function goToOrders()  {
    return navigateTo({ path: "/orders" })
}
export function goToNotifications()  {
    return navigateTo({ path: "/notifications" })
}
export function goToSettings()  {
    return navigateTo({ path: "/settings" })
}
export function goToMyShop()  {
    return navigateTo({ path: "/myshop" })
}
export function goToShop()  {
    return navigateTo({ path: "/shop" })
}
export function goToScanner()  {
    return navigateTo({ path: "/scanner" })
}
export function goToNehanda()  {
    return navigateTo({ path: "/nehanda" })
}

export function goToProduct() {
    return navigateTo({ path: "/product" })
}

export function goToCart() {
    return navigateTo({ path: "/cart" })
}

export function goToStats() {
    return navigateTo({ path: "/stats" })
}

export function goToAddProduct() {
    return navigateTo({ path: "/products/add" })
}

export function goToAdCreate() {
    return navigateTo({ path: "/ad" })
}

export function goToOrderDetails() {
    return navigateTo({ path: "/order" })
}

export function goToPerformances() {
    return navigateTo({ path: "/performances" })
}

export function goToPremium() {
    return navigateTo({ path: "/premium" })
}