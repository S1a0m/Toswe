      ...
      <div class="flex justify-between items-center">
        ...
        <div class="flex items-center space-x-3 cursor-pointer" @click="goToMarket">
          ...
          <div class="flex flex-col text-xs"> 
            ...
            <span class="text-xs text-gray-800 flex items-center gap-1" v-if="auth.isSeller"> 
              <Icon name="uil:shopping-cart" size="16" /> Vendeur
            </span> 
            <!--<span class="text-xs text-gray-800 flex items-center gap-1" v-if="auth.isDeliverer"> 
              <Icon name="uil:truck" size="16" /> Livreur
            </span>-->
          </div>
        </div>
          ...
          <template v-else>
            ...
            <!--<template v-if="auth.isDeliverer">
              <button
                :class="[
                  'flex items-center gap-2 p-3 rounded-full transition hover:scale-105 focus:ring-2 focus:ring-yellow-400/50',
                  isActive('/deliveries') ? 'bg-white/20 text-yellow-300 shadow-inner' : 'hover:bg-white/10'
                ]"
                aria-label="Mes livraisons"
                @click="goToMyDeliveries"
              >
                <Icon name="uil:truck" size="20" />
              </button>
            </template>-->
          </template>
        </nav>
          ...
          <template v-if="auth.isSeller">
            ...
            <!--<template v-if="auth.isDeliverer">
              ...
              class="flex items-center gap-3 px-3 py-3 rounded-md hover:bg-[#6B1F0D]/10 transition"
                @click="goToMyDeliveries"
              >
                <Icon name="uil:truck" size="20" class="text-[#6B1F0D]" />
                Mes livraisons
              </button>
            </template>-->
        </template>
...
const { goToMyShop, goToMyDeliveries } = useNavigation();
