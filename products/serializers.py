...
class ProductSearchSerializer(serializers.ModelSerializer):
    """
    Sérialiseur utilisé par search_products et Nehanda.
    Inclut tous les champs nécessaires pour construire un ProductCard.
    """
    ...
    def get_total_rating(self, obj):
        stats = obj.feedback_set.aggregate(
...
class SellerProductSerializer(serializers.ModelSerializer):
    """
    Sérialiseur riche pour la vue vendeur — inclut images,
    promotion active, stats de vente et note moyenne.
    """
    main_image = serializers.SerializerMethodField()
    ...
    def get_main_image(self, obj):
        request = self.context.get("request")
        img = obj.images.filter(is_main_image=True).first() or obj.images.first()
        if not img:
            return None
        return request.build_absolute_uri(img.image.url) if request else img.image.url
 
    def get_active_promotion(self, obj):
        promo = obj.promotions.filter(ended_at__gte=timezone.now()).first()
        if not promo:
            return None
        return ActivePromotionSerializer(promo).data
 
    def get_rating(self, obj):
        agg = Feedback.objects.filter(product=obj).aggregate(
            avg=Avg("rating"), count=Count("id")
        )
        return {
...
class SellerOfferSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            ...
            "duration_days", "commission_percent",
            "is_recommended", "features", "is_active",
        ]
...
class OfferSubscriptionSerializer(serializers.ModelSerializer):
    offer = SellerOfferSerializer(read_only=True)
 
    class Meta:
        model = OfferSubscription
        fields = [
            "id", "offer", "status", "payment_method",
            "phone_number", "amount_paid",
            "subscribed_at", "starts_at", "expires_at",
        ]